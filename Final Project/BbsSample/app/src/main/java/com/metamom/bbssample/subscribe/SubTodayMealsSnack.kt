...
class SubTodayMealsSnack : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        /* 액션바 설정 */
        setSupportActionBar(findViewById(R.id.subSnack_toolbar))
        ...
        findViewById<Toolbar>(R.id.subSnack_toolbar).setNavigationOnClickListener {
            onBackPressed()
        }
        ...
        *  2-1) 불러온 식단 정보는 식단 추천 이력 확인을 위하여 REMEBER TABLE에 저장 */
        ...
        // 2) 식단 추천 이력이 없을 경우
            ...
            Log.d("SubTodayMealsSnack", "#21# SubTodayMealSingleton 확인 > ${SubTodayMealSingleton.toString()}")

            // 2-i) [다이어트] 회원이 다이어트 식단을 신청하였을 경우 (type == 0)
            if (SubTodayMealSingleton.type == 0) {
                Log.d("SubTodayMealsSnack", "#21# 오늘의 식단[아침] *다이어트 식단* 가져오기 실행")
                ...
                if (todayMeal != null) {
                    ...
                    if (rememberMeal == "Success") Log.d("SubTodayMealsSnack", "#21# 추천한 다이어트 식단 REMEMBER TABLE 내 저장완료")
                }
                else {
                    Toast.makeText(this, "관리자에게 문의해주시길 바랍니다. 죄송합니다", Toast.LENGTH_LONG).show()
                    Log.d("SubTodayMealsLunch", "#21# 오늘의 식단 [간식] *다이어트* 해당되는 식단 없음(null) Error 발생")
            ...
            // 2-i) [운동] 회원이 운동 식단을 신청하였을 경우 (type == 1)
                ...
                Log.d("SubTodayMealsSnack", "#21# 오늘의 식단[아침] *운동 식단* 가져오기 실행")
                ...
                if (todayMeal != null) {
                    ...
                    if (rememberMeal == "Success") Log.d("SubTodayMealsSnack", "#21# 추천한 운동 식단 REMEMBER TABLE 내 저장완료")
                }
                else {
                    Toast.makeText(this, "관리자에게 문의해주시길 바랍니다. 죄송합니다", Toast.LENGTH_LONG).show()
                    Log.d("SubTodayMealsLunch", "#21# 오늘의 식단 [간식] *운동* 해당되는 식단 없음(null) Error 발생")
        ...
        if (todayMeal != null){
            Log.d("SubTodayMealsSnack", "#21# Back으로부터 전달받은 오늘의 식단 > ${todayMeal.toString()}")
